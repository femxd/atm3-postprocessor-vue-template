 ...
 * @type {Array}
 */
var TEXT_FILE_EXTS = ['css', 'tpl', 'js', 'php', 'txt', 'json', 'xml', 'htm', 'text', 'xhtml', 'html', 'md', 'conf', 'po', 'config', 'tmpl', 'coffee', 'less', 'sass', 'jsp', 'scss', 'manifest', 'bak', 'asp', 'tmp', 'haml', 'jade', 'aspx', 'ashx', 'java', 'py', 'c', 'cpp', 'h', 'cshtml', 'asax', 'master', 'ascx', 'cs', 'ftl', 'vm', 'ejs', 'styl', 'jsx', 'handlebars', 'shtml', 'ts', 'tsx'],
    IMAGE_FILE_EXTS = ['svg', 'tif', 'tiff', 'wbmp', 'png', 'bmp', 'fax', 'gif', 'ico', 'jfif', 'jpe', 'jpeg', 'jpg', 'woff', 'cur', 'webp', 'swf', 'ttf', 'eot', 'woff2'],
    MIME_MAP = {
  //text
  'css': 'text/css',
  'tpl': 'text/html',
  ...
  if (source) {
    ...
    if (stat.isDirectory()) {
      fs.readdirSync(source).forEach(function (name) {
        if (name != '.' && name != '..') {
          removedAll = _.copy(source + '/' + name, target + '/' + name, include, exclude, uncover, move) && removedAll;
        }
      });
      if (move && removedAll) {
        fs.rmdirSync(source);
  ...
  if (processors) {
    ...
    processors.forEach(function (obj, index) {
      ...
      if (typeof processor === 'function') {
        ...
        (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object' && _.assign(settings, obj);
        ...
        console.warning('invalid processor [modules.' + key + ']');
      }
    });
  }
};
...
_.download = function (url, callback, extract, opt) {
  ...
  var writer = fs.createWriteStream(tmp),
      http_err_handler = function http_err_handler(err) {
    writer.destroy();
    fs.unlinkSync(tmp);
    var msg = (typeof err === 'undefined' ? 'undefined' : _typeof(err)) === 'object' ? err.message : err;
    if (callback) {
      callback(msg);
    } else {
    ...
    res.on('data', function (chunk) {
      ...
      if (status >= 200 && status < 300 || status === 304) {
        if (extract) {
          fs.createReadStream(tmp).pipe(getTar().Extract({
            path: extract
...
_.upload = function (url, opt, data, content, subpath, callback) {
  if (typeof content === 'string') {
    ...
    console.error('unable to upload content [%s]', typeof content === 'undefined' ? 'undefined' : _typeof(content));
  }
  opt = opt || {};
  data = data || {};
  var endl = '\r\n';
  ...
  matches.forEach(function (item, index) {
    ...
    if (match !== item.negate) {
      ...
      keys.forEach(function (key) {
        ...
        var value = _typeof(properties[key]) === 'object' ? fis.util.cloneDeep(properties[key]) : properties[key];
