...
describe("posthtml demo", function () {
  ...
  it("demo1", () => {
    var html = vueParser.parse("./store-index.html");
    _.write("dest/store-index.html", html);
  });
});
...
function VueParser(options = {modBaseDir: ""}) {
  ...
  return {
    parse: function handleEntryHtml(file) {
      log.allan("[1] start handle entry html: ", file);
      var $ = cheerio.load(_.read(file), {
        decodeEntities: false
      });
      var comps = $('component');
      ...
      log.allan("[1] success handled entry => ", file);
      var raw = $.html();
